...
function buildStationParams(scope = "turkiye") {
  ...
  try {
    const response = await fetchImpl(url, {
      ...
      headers: {
        ...
        "User-Agent": "Yakinim/3.1.4 (+https://yakinim.vercel.app)",
      },
    });
...
async function queryRadio(scope = "turkiye", fetchImpl = fetch) {
  ...
  for (const server of RADIO_SERVERS) {
    try {
      ...
      const stations = uniqueStations(rows);
...
module.exports = { RADIO_SERVERS, RADIO_SCOPES, buildStationParams, cleanProviderText, normalizeArtworkUrl, normalizeStation, uniqueStations, queryRadio, countStationClick };
