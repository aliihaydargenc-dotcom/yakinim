...
assert.match(index, /<span>ÇEVRE ÖZETİ<\/span>/);
assert.match(index, /id="localLifeHeading"/);
assert.match(index, /https:\/\/www\.antalya\.bel\.tr\/tr\/etkinlikler/);
assert.match(index, /Kaynak: Antalya Büyükşehir Belediyesi/);
assert.match(index, /Tarih ve saat için kaynağı doğrula/);
assert.match(styles, /verified local-life entry point/);
