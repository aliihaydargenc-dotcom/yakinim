...
replies=[{ok:true,json:async()=>({elements:[],remark:'timeout'})},ok([]),ok([{id:2}])];calls=[];
assert.equal((await queryNearby({lat:36.89,lng:30.70},1000,fake)).elements[0].id,2);assert.equal(calls.length,3);
replies=[new Error('offline'),new Error('offline'),new Error('offline')];
